...
parser.add_argument(
    "mode",
    choices=["configure", "progress", "cmake"],
    default="configure",
...
parser.add_argument(
    "--check",
    action="store_true",
    help="(cmake mode) verify the generated file is up to date instead of writing it",
)
...
cmake_sources_path = Path("cmake") / "sources.cmake"


def format_cmake_sources() -> str:
    config.validate()

    sources: dict[str, list[str]] = {name: [] for name in cmake_categories.values()}
    ...
    for obj in config.objects().values():
        ...
        if isinstance(category, list):
            # Assign multi-category objects to the first exported category
            category = next((c for c in category if c in cmake_categories), None)
        if category not in cmake_categories:
            continue

        if obj.src_path is None or not file_is_c_cpp(obj.src_path):
            continue
        if not obj.src_path.exists():
            missing.append(obj.src_path)
            continue

        path = obj.src_path.as_posix()
        sources[cmake_categories[category]].append(path)
    ...
    out = [
        "# Generated by `python configure.py cmake`. Do not edit.",
        "#",
        "# Source lists for the CMake (PC/aurora) build, derived from the object",
        "# lists in configure.py so the two build systems cannot drift apart.",
        "# Regenerate after adding, removing or recategorizing any object.",
        "",
    ]

    def add_list(name: str, paths: list[str]) -> None:
    ...
    for name, paths in sources.items():
        add_list(f"MELEE_SOURCES_{name}", paths)

    out.append("# Units that do not match the original DOL. These are still built")
    out.append("# for PC; the list is exported for reporting only.")
    add_list("MELEE_SOURCES_NONMATCHING", non_matching)

    return "\n".join(out)


def generate_cmake_sources() -> None:
    contents = format_cmake_sources()

    if args.check:
        if (
            not cmake_sources_path.exists()
            or cmake_sources_path.read_text(encoding="utf-8") != contents
        ):
            sys.exit(
                f"{cmake_sources_path} is out of date."
                "\nRun `python configure.py cmake` and commit the result."
            )
        print(f"{cmake_sources_path} is up to date")
        return

    cmake_sources_path.parent.mkdir(parents=True, exist_ok=True)
    cmake_sources_path.write_text(contents, encoding="utf-8")
    print(f"Wrote {cmake_sources_path}")
